        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {subjects.map(({ slug, emoji, label, rq, metric }) => (
            <Link
              key={slug}
              to={`/sciences/${slug}`}
              className="border border-slate-200 rounded-2xl p-8 shadow-sm hover:-translate-y-0.5 hover:shadow-md hover:border-rose-300 transition-all duration-200 block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500 focus-visible:ring-offset-2"
            >
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl">{emoji}</span>
                <h2 className="text-xl font-bold text-slate-950">{label}</h2>
              </div>
              <p className="text-base text-rose-500 font-medium italic mb-4 leading-snug">{rq}</p>
