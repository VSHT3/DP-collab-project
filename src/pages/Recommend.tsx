...
export default function Recommend() {
  ...
  return (
    <div className="px-8 lg:px-16 py-16">
      ...
      <div className="mb-10">
        <span className="text-sm font-semibold tracking-widest text-rose-500 uppercase">Recommendation Tool</span>
        <h1 className="text-4xl font-bold text-slate-950 mt-2 mb-3">Find Your Best Product</h1>
            ...
            <div key={key}>
              <div className="flex justify-between items-baseline mb-2">
                <div>
                  ...
                  <span className="ml-2 text-sm text-slate-600">{description}</span>
                </div>
      ...
      {submitted && (
        <div className="space-y-4">
          ...
          >
            <p className="text-base font-semibold opacity-80 mb-1">Best match for you</p>
            <h2 className="text-3xl font-bold mb-1">{products[winner.key].label}</h2>
            <p className="text-base opacity-80">{products[winner.key].brand}</p>
          ...
          <div className="border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            ...
            {ranked.map(({ key, score, hasPending }, i) => (
              <div key={key} className="flex items-center justify-between px-6 py-5 border-t border-slate-100 hover:bg-slate-50/50 transition-colors duration-200">
                <div className="flex items-center gap-3">
                  <span className="text-slate-400 text-base font-bold w-6">{i + 1}</span>
                  <span className="w-3.5 h-3.5 rounded-full" style={{ background: products[key].color }} />
                  <div>
                    <span className="text-base font-medium text-slate-900">{products[key].label}</span>
                    {hasPending && <span className="text-sm text-amber-500 ml-1 font-medium">*</span>}
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-base font-medium text-slate-800 w-12 text-right">{score.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
