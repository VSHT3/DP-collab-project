...
function computeScores(weights: Weights): { key: ProductKey; score: number }[] {
  const totalWeight = Object.values(weights).reduce((a, b) => a + b, 0) || 1
  const productKeys = Object.keys(products) as ProductKey[]
  return productKeys
    .map(key => {
      const score = axes.reduce((sum, axis) => {
        return sum + (weights[axis.key] / totalWeight) * products[key].scores[axis.key]
      }, 0)
      return { key, score }
    })
    ...
    <div className="max-w-3xl mx-auto px-6 py-16">
      <div className="mb-10">
        ...
        <p className="text-slate-500">
          Drag the sliders to reflect what matters most to you. The tool will
          calculate your best-matched menstrual product based on our research data.
        </p>
      </div>
      ...
      {submitted && (
        <div className="space-y-4">
          ...
          >
            ...
            <h2 className="text-2xl font-bold mb-2">{products[winner.key].label}</h2>
          ...
          <div className="border border-slate-100 rounded-2xl p-6">
            <h3 className="font-semibold text-slate-900 mb-4">Score Breakdown — {products[winner.key].label}</h3>
            <div className="space-y-3">
              {axes.map(({ key, label }) => {
                const raw = products[winner.key].scores[key]
                const weight = weights[key]
                return (
                  <div key={key} className="flex items-center justify-between text-sm">
                    <span className="text-slate-600">{label}</span>
                    <div className="flex items-center gap-4">
                      <span className="text-slate-400 text-xs">score {raw.toFixed(1)} × priority {weight}</span>
                      <span className="font-medium text-slate-800 w-12 text-right">
                        {(raw * weight).toFixed(1)}
                      </span>
                    </div>
                  </div>
                )
